...
```{r}
#| eval: false
# ag: har ikke så lyst til å se over 30000 rekker med befolkningsdata
# fra eu. Har derfor satt eval lik false. Et enkelt alternativ er å benytte paged.print: false
# kan også settes i setup chunk vha: options(paged.print = FALSE)
pop
```
...
# ag: Kode som viser hvordan vi kan få til to kompakte tabeller som viser dataene for
# Albania. Selv dette tar to sider og er ikke veldig spennende lesning ;-)
pop |> 
  head(n = 126) %>%
  {
    list(
...
```{r}
#| label: tbl-six-zones
#| tbl-cap: Table of GDP data for six random NUTS3 zones in the dataset.
#| message: false
# ag: det er over 30000 observasjoner. Ingen god ide å printe alle 
# disse i dokumentet. Et alternativ kan være å plukke ut f.eks 6 NUTS3
# soner og presentere disse
{
  set.seed(13012026)
  nuts3_sample <- gdp |> 
    arrange(geo, time) %>% 
...
# ag: stor tabell. Notorisk vanskelig å få fint formatert
# særlig for flere format. Her er mitt forsøk.
gdp |> 
  filter(geo %in% nuts3_sample) |> 
  arrange(geo, time) %>% 
  mutate(gdp_n3 = gdp_n3/1000) |> 
  complete(geo, time, fill = list(NA)) %>% 
  {
    list(
      p1 = slice(., 1:48),
      p2 = slice(., 49:96),
  ...
  bold(i = c(1,25)) |> 
  align(j = c(3, 6, 9), align = "left", part = "header") |> 
  align(j = c(3, 6, 9), align = "center", part = "body") |> 
  vline(j = c(3,6)) |> 
  set_header_labels(
    geo...1 = "nuts3",
    time...2 = "År",
  ...
  width(j = c(1, 4, 7), width = 0.45) |> 
  width(j = c(2, 5, 8), width = 0.2) |> 
  width(j = c(3, 6, 9), width = 0.5)
}
```
